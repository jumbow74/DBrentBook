...
if(isset($_POST['Comfirm'])){
	$query = "INSERT INTO AA_BOOK (ISBN, BOOKNAME, PRICERENT, TOTAL, REMAIN, ID_TYPE, ID_COMPANY, WRITER, NUM_PAGE, DETAIL , DATE_ADD) VALUES ('".$_POST['isbn_book']."', '".$_POST['name_book']."', '".$_POST['price_book']."', '".$_POST['number_book']."', '".$_POST['number_book']."', '".$_POST['type_book']."', '".$_POST['company_book']."', '".$_POST['name_create_book']."', '".$_POST['num_book']."' , '".$_POST['details_book']."',sysdate)";
	$parseRequest = oci_parse($conn, $query);
	if(oci_execute($parseRequest)){
	$i=0;
	while($i<$_POST['number_book']){
	$query = "INSERT INTO AA_EACHBOOK (ID, STATUSBOOK, STATISTICBOOK, ISBN) VALUES (id_eachbook_seq.nextval, '1', '0', '".$_POST['isbn_book']."')"; 
	$parseRequest = oci_parse($conn, $query);
...
<form action='addBooking.php' method='POST' data-ajax='false' enctype="multipart/form-data" >
		...
		<td align="left" width="60%" class="ui-body ui-body-a ui-corner-all">
			...
			<td>
				...
				รหัส ISBN
				<input type="text"  name="isbn_book" id="isbn_book" value="" placeholder="xxx-xxx-xx-xxxx-x" required >
				ชื่อหนังสือ
